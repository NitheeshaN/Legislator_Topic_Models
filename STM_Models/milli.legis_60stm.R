...
saveRDS(tokens_leg, file="tokens_leg60.rds") #116MB

dfm_leg <- dfm(tokens_leg, tolower=TRUE) #117MB 779288

saveRDS(dfm_leg, file="full_dfm_leg60.rds")
...
saveRDS(dfm_sm_leg, file="dfm_sm_leg60.rds")

dfm_sm_leg_stm <- convert(dfm_sm_leg,to="stm") #25M

saveRDS(dfm_sm_leg_stm, file="dfm_sm_leg_stm60.rds")
...
saveRDS(stm_sm_leg,file="stm_sm_leg60.RDS") #8M
...
cat("Small Model, By Legislator\n", file="stm_sm_leg60_kw.txt", append=FALSE)
for (k in 1:60) {
  cat(paste("Topic ",k,":\n", sep=""), file="stm_sm_leg60_kw.txt", append=TRUE)
  ...
  cat("\n", file="stm_sm_leg60_kw.txt", append=TRUE)
}
