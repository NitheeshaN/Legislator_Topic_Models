...
saveRDS(tokens_twts, file="tokens_twts120.rds") #116MB

dfm_twts <- dfm(tokens_twts, tolower=TRUE) #117MB 779288

saveRDS(dfm_twts, file="full_dfm_twts120.rds")
...
saveRDS(dfm_sm, file="dfm_sm_twts120.rds")

dfm_sm_stm <- convert(dfm_sm,to="stm") #25M

saveRDS(dfm_sm_stm, file="dfm_sm_twts_stm120.rds")
...
saveRDS(stm_sm_twts,file="stm_sm_twts120.RDS") #8M
...
cat("Small Model, By Legislator\n", file="stm_sm_twts120_kw.txt", append=FALSE)
for (k in 1:120) {
  cat(paste("Topic ",k,":\n", sep=""), file="stm_sm_twts120_kw.txt", append=TRUE)
  ...
  cat("\n", file="stm_sm_twts120_kw.txt", append=TRUE)
}
